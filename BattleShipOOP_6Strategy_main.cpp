...
// Bo nho phu cho Target mode. Dung lai cho HuntTarget va Hybrid.
struct TargetMemory {
    bool targetMode;
...
// Hybrid Adaptive:
// - Hunt mode: checkerboard + center bias.
// - Target mode: giong HuntTarget.
class HybridAdaptiveStrategy : public Strategy {
private:
    TargetMemory targetMemory;
    vector<Position> checkerCenterOrder;

    void buildCheckerCenterOrder() {
        checkerCenterOrder.clear();

        for (int r = 0; r < BOARD_SIZE; r++) {
            for (int c = 0; c < BOARD_SIZE; c++) {
                if ((r + c) % 2 == 0) {
                    checkerCenterOrder.push_back(Position(r, c));
                }
            }
        }

        sort(checkerCenterOrder.begin(), checkerCenterOrder.end(), [this](const Position& a, const Position& b) {
            return centerDistanceScore(a) < centerDistanceScore(b);
        });
    }
...
public:
    ...
    Position chooseShot(mt19937& rng) {
        ...
        // Khong con target hop le thi quay lai Hunt mode ket hop caro + gan tam.
        targetMemory.reset();

        for (size_t i = 0; i < checkerCenterOrder.size(); i++) {
            if (canShoot(checkerCenterOrder[i])) {
                markFired(checkerCenterOrder[i]);
                return checkerCenterOrder[i];
            }
        }

        // Khi het o caro, ban random cac o con lai.
        Position p = chooseRandomUnfired(rng);
        markFired(p);
        return p;
    }
    ...
    Strategy* clone() const {
        return new HybridAdaptiveStrategy(*this);
    }
};
...
public:
    Simulation(int games = 100000) {
        ...
        strategies.push_back(new HybridAdaptiveStrategy());
